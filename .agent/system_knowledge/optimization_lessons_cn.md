...
## 8. 制度级一致性：多 API 对齐与原子化镜像 (Institutional Parity)

### **核心挑战**：
在大规模搜索中，`Turbo` 模式为了追求极致速度，容易在数据对齐和全局状态管理上牺牲精度，导致“0 交易”或结果漂移。

### **实战经验**：
- **原子化镜像 (Atomic Mirroring)**：多进程竞争写入 K 线缓存时，必须采用 `tempfile + rename`。直接写入会导致文件损坏，进而触发回测静默失败（0 交易）。
- **Payload 瘦身**：Single API 的 500 错误往往由于 JSON 序列化巨大对象（数十万笔订单）导致。批量任务中，只返回摘要统计（Results）是保证系统稳定的关键。
- **强制 Fast Path**：所有接口（包括 Single）必须强制对齐使用相同的磁盘/内存镜像路径。路径不一致是回测结果细微差异（如万分之一 PnL 偏差）的元凶。

---
**当前状态**：🏆 48 核满载优化达成 (4668%) | API 100% 数据对齐 | 原子化缓存镜像 | 回测 payload 动态拓扑优化完成。
