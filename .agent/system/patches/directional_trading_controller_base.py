...
class DirectionalTradingControllerBase(ControllerBase):
    ...
    def create_actions_proposal(self) -> List[ExecutorAction]:
        ...
        if signal != 0 and self.can_create_executor(signal):
            ...
            # ⚡ MEGA-TURBO FIX: Ensure price is Decimal to avoid TypeError with float from DataFrame
            amount = self.config.total_amount_quote / Decimal(str(price)) / Decimal(self.config.max_executors_per_side)
