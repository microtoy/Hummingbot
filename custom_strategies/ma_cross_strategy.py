...
class MACrossStrategyConfig(DirectionalTradingControllerConfigBase):
    ...
    indicator_interval: CandleInterval = Field(default=CandleInterval.H1, description="Candles interval for MA calculation")
...
class MACrossStrategyController(DirectionalTradingControllerBase):
    ...
    async def update_processed_data(self):
        ...
        if candles is None or candles.empty:
            import pandas as pd
            print(f"⚠️ [DATA ERROR] No candles found for {self.config.trading_pair} at {interval}")
        ...
        # --------------------------------------------------------------------------
        # [ANTI-LOOK-AHEAD BIAS] Indicator Lagging Logic
        # We calculate metrics on current candles, but SHIFT the signal by 1.
        # This ensures that a signal generated by a candle that CLOSED at T
        # is only acted upon at T+1 (the next resolution step).
        # --------------------------------------------------------------------------
        ...
        candles["signal"] = 0
        # Determine raw signals based on crosses
        candles.loc[(candles["fast_ma"] > candles["slow_ma"]) & (candles["fast_ma"].shift(1) <= candles["slow_ma"].shift(1)), "signal"] = 1
        candles.loc[(candles["fast_ma"] < candles["slow_ma"]) & (candles["fast_ma"].shift(1) >= candles["slow_ma"].shift(1)), "signal"] = -1

        # IMPORTANT: Shift signals forward 1 row to prevent using future close prices
